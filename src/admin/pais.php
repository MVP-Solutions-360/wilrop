...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errores)) {
        $sql = 'INSERT INTO paises (pais, descripcion) VALUES (?, ?, ?)';
        $stmt = mysqli_prepare($conexion, $sql);
        if ($stmt) {
            $pais_val = strtolower($pais);
            mysqli_stmt_bind_param($stmt, 'sss', $pais_val, $descripcion);
            if (mysqli_stmt_execute($stmt)) {
                ...
                $errores[] = 'Error al crear el pais: ' . htmlspecialchars(mysqli_error($conexion));
            }
    ...
    <section class="admin-section">
        <div class="container">
            <div class="section-header">
                ...
                <p>Gestiona los paises que ofrecerás en tu sitio</p>
            </div>
            <div class="admin-content">
                <!-- Formulario de Nuevo Destino -->
                <div class="form-container admin-form-container">
                    <h3><i class="fas fa-plus-circle"></i> Agregar Nuevo pais</h3>
                    ...
                    <?php endif; ?>
                    <form id="paisForm" class="admin-form" method="POST" action="pais.php">
                        <div class="form-row">
                            <div class="form-group">
                                <label for="pais">Pais<span style="color:#e74c3c">*</span></label>
                                <input type="text" id="pais" name="pais" maxlength="100" required placeholder="Ej: Santo Domingo, Medellín" value="<?php echo htmlspecialchars($pais); ?>">
                            </div>
                        </div>
                        <div class="form-group">
                            <label for="descripcionDestino">Descripción</label>
                            <textarea id="descripcionDestino" name="descripcion" rows="4" placeholder="Describe detalladamente el pais..."><?php echo htmlspecialchars($descripcion); ?></textarea>
                        </div>
